...
class Game {
  ...
  levelTwoMissileExplosionCollision(i) {
    this.explosions.forEach((explosion) => {
  ...
  wins(levelArray) {
    if (this.Missiles.length === 0 && this.score > 0 && this.Cities.length > 1) {
      this.level += 1;
  ...
  transitions() {
    ...
    if (this.level === 2 && this.Cities.length > 0 && this.levelTwo.length === 0 && this.score > 0) {
      console.log(this.levelBeaten)
      this.gameWon = true;
      this.gameWonScreen();
      return true;
    }
